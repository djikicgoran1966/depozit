...
import { GETKARTICEBYSUM, KARTICE, NEGATIVEKARTICE } from '../../queries/kartice'
...
import { FINDSTAVKEBYKARTICA, GETSUM, PRETRAGA } from '../../queries/stavke';
...
export default function EditNalogModal({closeMenu,nalog}) {
  ...
  const [ updateStavka , {error:errorStavka,data:dataStavka}]=useMutation(UPDATESTAVKA,{ refetchQueries:[PRETRAGA, NEGATIVEKARTICE, GETKARTICEBYSUM,GETSUM  ]})

  if (loading) return <CircularProgress />
  if (loadingOznake) return <CircularProgress />
  ...
  const handleSubmit=(e)=>{
    ...
    updateStavka( { variables:{ id:nalog._id, newStavka:{godina,broj_izvoda,datum,kartica:kartica===null ? "":kartica._id,korisnik,deponent,oznaka: oznaka===null ? "64dcad2c93eddbd5eb3a32ed":oznaka._id  ,broj_predmeta,godina_predmeta,duguje,potrazuje} } } )
    .then(stavka=>{
