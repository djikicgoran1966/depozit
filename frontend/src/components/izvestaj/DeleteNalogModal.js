...
import { GETKARTICEBYSUM, NEGATIVEKARTICE } from '../../queries/kartice';
...
export default function DeleteNalogModal({closeMenu, nalog}) {
  ...
  const [ deleteStavka, {data, loading, error} ] =useMutation(DELETESTAVKA,{refetchQueries:[ NEGATIVEKARTICE, GETKARTICEBYSUM,GETSUM ]})
    ...
    <div>
      ...
      >
        ...
        { newQuery && error && <pre>  { error.graphQLErrors.map(({ message},i)=> < Alert key={i} severity="error">  { message} </Alert> )} </pre> }
          ...
          <Typography sx={{ textAlign:"center", fontWeight:100}} id="modal-modal-title"   >
