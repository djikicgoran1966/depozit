...
export default function DeleteUserModal({closeMenu, user}) {
  ...
  const handleDeleteUser=()=>{
    setNewQuery(true)
    deleteUser( { variables:{id:user._id} }, { refetchQueries:[
        GET_USERS
    ]})
    .then(result=>{
        console.log(result)
  ...
  const [ deleteUser, {data, loading, error} ] =useMutation(DELETE_USER, { refetchQueries:[
    GET_USERS
  ] }) 
      ...
      >
        ...
        { newQuery && error && <pre>  { error.graphQLErrors.map(({ message},i)=> < Alert key={i} severity="error">  { message} </Alert> )} </pre> }
          ...
          <Typography size="small" sx={{ textAlign:"center"}} id="modal-modal-title"   >
            Da li ste sigurni da želite da obrišete korisnika : <Box sx={{ fontWeight:"bold", textAlign:"center" }}> { user.firstName} { user.lastName} ?</Box> 
          </Typography>
           <Box sx={{
             mt:2,
             display:"flex",
            ...
            <Button onClick={ handleDeleteUser } size="small" sx={{ mr:4}} variant="outlined" color="inherit"> Da</Button>
